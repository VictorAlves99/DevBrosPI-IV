...
public class VendaDAO extends DAO<Venda> {
	...
	public List<Venda> getListaPorData(Date dataInicio, Date dataFim, Usuario usuario) throws MensagemException {
		if(dataInicio == null || dataFim == null) {	
			throw new MensagemException(Mensagens.getMensagem("Preencha corretamente as datas de Início e Fim!"));
		}
		...
		try {
			...
			query.select(root).where(builder.equal(root.get("cliente"), usuario),
					builder.between(root.<Date>get("dataVenda"), dataInicio, dataFim));
